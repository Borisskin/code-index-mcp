...
4. Журналы: `C:/tools/code-index/daemon.log` (демон) и `C:/tools/code-index/serve.log`
   (сервер выдачи). Ротация по 10 МБ: `daemon.log.1` … `.3`.

   Разбор «встало на индексации» по журналу:

   ```bash
   # на чём остановились: последняя строка этапа перед тишиной
   grep -E "этап|начат|итог первичной" C:/tools/code-index/daemon.log | tail -20
   # жив ли процесс и что стоит без движения
   grep "\[пульс\]" C:/tools/code-index/daemon.log | tail -10
   ```

   Растущее «без изменений N» в пульсе при неменяющемся этапе — это и есть
   зависание на названном этапе. Подробность поднимается `[daemon] log_level`
   в `daemon.toml` либо разово переменной `RUST_LOG` (она сильнее конфига);
   на `debug` добавляются сообщения по отдельным файлам.

   Пока демон держит файл открытым, Windows показывает нулевой размер журнала —
   смотреть надо содержимое, а не размер.
