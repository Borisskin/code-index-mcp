...
### Перечитка `serve.toml` без перезапуска (v1.1.0)

В режиме федерации `serve` сам следит за `serve.toml` и `daemon.toml`: через полсекунды после сохранения таблица репозиториев перестраивается целиком, и новый алиас начинает обслуживаться без перезапуска — уже открытые MCP-сессии клиентов видят его сразу. Правка применяется по принципу «всё или ничего»: ошибка разбора, неоткрывшаяся база или локальный алиас из `serve.toml` без записи в `daemon.toml` оставляют прежнюю таблицу. Применить вручную — `POST /reload` (защищён тем же списком разрешённых адресов), итог последней перечитки — поле `config_reload` инструмента `health`. Смена `[me].ip` и `[pool]` по-прежнему требует перезапуска. Демон тоже сам следит за `daemon.toml`: новый каталог начинает индексироваться, убранный перестаёт отслеживаться без перезапуска — `daemon reload` остаётся ручным запасным путём.
